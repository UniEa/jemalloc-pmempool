...
/******************************************************************************/
/* Data. */
extern pmempool_t pmempool;  ///持久性内存池
...
/*持久性内存池版本*/
void //*
pp_chunk_alloc(size_t size, size_t alignment, bool base, bool *zero,
    dss_prec_t dss_prec, arena_chunk_t ** chunk)
{
	void * ret = pmempool_chunk_alloc(&pmempool);
	*chunk = ret;
	//return ret;
}
...
/*持久性内存池版本*/
void
pp_chunk_dealloc(void *chunk, size_t size, bool unmap)
{
	pmempool_free(&pmempool, chunk);
}
